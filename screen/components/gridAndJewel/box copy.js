...
export default class Box extends React.Component {
  ...
  buildGrid = () => {
    for (let i = 0; i < this.state.gridHeigth; i++) {
      for (let j = 0; j < this.state.gridWidth; j++) {
        if ((i + j) % 2 == 0) {
          this.state.line.push(<Text style={styles.gridBox}></Text>);
        } else {
  ...
  buildJewel = () => {
    ...
    for (let i = 0; i < this.state.gridHeigth; i++) {
      for (let j = 0; j < this.state.gridWidth; j++) {
        if (
          ((this.props.hint[0][0] = i) && (this.props.hint[0][1] = j)) ||
          ((this.props.hint[1][0] = i) && (this.props.hint[1][1] = j))
        ) {
          this.state.jewel.push(
            <TouchableOpacity style={styles.yesHint}>
              <Image
                onTouchStart={(e) => {
                ...
                onTouchEnd={(e) => {
                  ...
                  if (this.state.end[0] - this.state.start[0] > offset) {
                    notdone = false;
                    this.props.swap([i, j], [i, j + 1]);
                  }
  
                  if (
                    this.state.end[0] - this.state.start[0] < -1 * offset &&
                    notdone
                  ) {
                    notdone = false;
                    this.props.swap([i, j], [i, j - 1]);
                  ...
                  if (
                    this.state.end[1] - this.state.start[1] > offset &&
                    notdone
                  ) {
                    notdone = false;
                    this.props.swap([i, j], [i + 1, j]);
                  }
  
                  if (
                    this.state.end[1] - this.state.start[1] < -1 * offset &&
                    notdone
                  ) {
                    this.props.swap([i, j], [i - 1, j]);
                  }
                ...
                style={styles.jewel}
                source={this.matrix[i][j] + 2}
              />
            </TouchableOpacity>
          );
        } else {
          this.state.jewel.push(
            <TouchableOpacity style={styles.noHint}>
              <Image
                onTouchStart={(e) => {
                  console.debug("TouchStart");
                  this.state.start = [e.nativeEvent.pageX, e.nativeEvent.pageY];
  ...
  render() {
    return (
      <View>
        <View style={styles.box}>{this.state.line}</View>
        <View style={styles.boxJewel}>{this.state.jewel}</View>
      </View>
    );
  }
}
